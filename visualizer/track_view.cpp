 ...
 *
 * Y-axis is exaggerated (default 50x) so lateral deviations are visible,
 * since the track is ~1000m long but only ~5m tall at true scale.
 *
...
static constexpr float Y_EXAG            = 1.f;     // Y exaggeration (1 = true scale)
static constexpr float INITIAL_VIEW_W    = 120.f;  // meters of x-axis visible at start
static constexpr float VEHICLE_RADIUS    = 2.0f;   // display-meters (exaggerated space)
...
static bool loadCSVFrames(const std::string& path, std::vector<Frame>& frames) {
    ...
    while (std::getline(file, line)) {
        ...
        while (std::getline(ss, field, ',')) {
            double val = 0;
            try { val = std::stod(field); } catch (...) { continue; }
            switch (col) {
                case 0:  f.time = val; break;
                case 1:  f.phi_dot = val; break;
                case 2:  f.beta = val; break;
                case 3:  f.delta = val; break;
                case 4:  f.delta_dot = val; break;
                case 5:  f.e_y = val; break;
                case 6:  f.e_y_dot = val; break;
                case 7:  f.act_out = val; break;
                case 8:  f.velocity = val; break;
                case 9:  f.x_track = val; break;
                case 10: f.y_track = val; break;
                case 11: f.xveh = val; break;
                case 12: f.yveh = val; break;
                case 13: f.rolling_perf = val; break;
                case 14: f.avg_perf = val; break;
                case 15: f.thresh_errors = static_cast<int>(val); break;
                case 16: f.critical = (val != 0); break;
                case 17: f.violated = (val != 0); break;
                case 18: f.ff_ref_0 = val; break;
                case 19: f.ff_ref_1 = val; break;
                case 20: f.trig_sens   = (val != 0); break;
                case 21: f.trig_net_sc = (val != 0); break;
                case 22: f.trig_est    = (val != 0); break;
                case 23: f.trig_ctrl   = (val != 0); break;
                case 24: f.trig_ff     = (val != 0); break;
                case 25: f.trig_merger = (val != 0); break;
                case 26: f.trig_net_ca = (val != 0); break;
                case 27: f.trig_act   = (val != 0); break;
            }
            col++;
        }
        if (col >= 13) frames.push_back(f);
    }
...
static sf::Color e_yToColor(double e_y, double threshold = 0.2) {
    ...
    if (ratio < 0.25f)
        ...
        return sf::Color(static_cast<sf::Uint8>(30 + 225 * (ratio - 0.25f) / 0.25f),
                         220, 30);
    else if (ratio < 0.75f)
        return sf::Color(255,
                         static_cast<sf::Uint8>(220 * (1.f - (ratio - 0.5f) / 0.25f)), 30);
    else
        return sf::Color(255, static_cast<sf::Uint8>(40 * (1.f - ratio)), 30);
}

// Convert world (x,y) to display coordinates (x, y*Y_EXAG)
static inline sf::Vector2f toDisp(double wx, double wy) {
    return sf::Vector2f(static_cast<float>(wx),
                        static_cast<float>(wy * Y_EXAG));
}

// Build a thick polyline as a triangle strip.
static void buildThickLine(const std::vector<sf::Vector2f>& pts,
                            float halfW, sf::Color color,
                            sf::VertexArray& out) {
    if (pts.size() < 2) return;
    out.setPrimitiveType(sf::TrianglesStrip);
    for (size_t i = 0; i < pts.size(); i++) {
        ...
        if (i == 0) {
            ...
            if (l1 < 1e-6f) d1 = d2; else d1 /= l1;
            if (l2 < 1e-6f) d2 = d1; else d2 /= l2;
...
static std::vector<sf::Vector2f> subsampleLocal(
    const std::vector<sf::Vector2f>& full, float xCenter, float xHalf, size_t maxPts)
{
    float xMin = xCenter - xHalf * 1.5f;
    float xMax = xCenter + xHalf * 1.5f;
    size_t count = 0;
    for (auto& p : full)
        if (p.x >= xMin && p.x <= xMax) count++;
    size_t skip = (count > maxPts) ? count / maxPts : 1;
    std::vector<sf::Vector2f> local;
    bool started = false;
    size_t idx = 0;
    for (auto& p : full) {
        if (p.x < xMin || p.x > xMax) { started = false; continue; }
        if (!started) { local.push_back(p); started = true; idx = 0; continue; }
        idx++;
        if (idx % skip == 0) local.push_back(p);
    }
    return local;
}
...
int main(int argc, char* argv[]) {
    ...
    // ── Load track data ──
    std::vector<double> track_x = loadCSVColumn(exampleDir + "/x_position_track.csv");
    std::vector<double> track_y = loadCSVColumn(exampleDir + "/y_position_track.csv");
    ...
    size_t fullSkip = (track_x.size() > TRACK_FULL_MAX) ? track_x.size() / TRACK_FULL_MAX : 1;
    ...
    if (!loadCSVFrames(csvFile, frames)) return 1;
    ...
    if (!hasFont) std::cerr << "WARNING: No monospace font found.\n";
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) { window.close(); }
                ...
                case sf::Keyboard::Escape: case sf::Keyboard::Q: window.close(); break;
                case sf::Keyboard::Space: paused = !paused; break;
                case sf::Keyboard::R: curFrame = 0; simTime = 0; break;
                case sf::Keyboard::F: followCar = !followCar; panOff = sf::Vector2f(0,0); break;
                case sf::Keyboard::Up:   playSpeed = std::min(playSpeed * 2.f, 64.f); break;
                case sf::Keyboard::Down: playSpeed = std::max(playSpeed / 2.f, 0.0625f); break;
                case sf::Keyboard::Right:
                    if (paused && curFrame < frames.size()-1) { curFrame++; simTime = frames[curFrame].time; } break;
                case sf::Keyboard::Left:
                    if (paused && curFrame > 0) { curFrame--; simTime = frames[curFrame].time; } break;
                default: break;
                }
            } else if (event.type == sf::Event::MouseWheelScrolled) {
                float factor = (event.mouseWheelScroll.delta > 0) ? 0.85f : 1.18f;
                viewW = std::clamp(viewW * factor, 2.f, 5000.f);
            } else if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Right) {
                    panning = true;
                    panStart = sf::Vector2f(static_cast<float>(event.mouseButton.x),
                                             static_cast<float>(event.mouseButton.y));
                }
            } else if (event.type == sf::Event::MouseButtonReleased) {
                if (event.mouseButton.button == sf::Mouse::Right) panning = false;
            } else if (event.type == sf::Event::MouseMoved) {
                if (panning && !followCar) {
                    float dx = static_cast<float>(event.mouseMove.x) - panStart.x;
                    float dy = static_cast<float>(event.mouseMove.y) - panStart.y;
                    panOff += sf::Vector2f(-dx * viewW / winW, dy * viewW / winW);
                    panStart = sf::Vector2f(static_cast<float>(event.mouseMove.x),
                                            static_cast<float>(event.mouseMove.y));
                }
            } else if (event.type == sf::Event::Resized) {
                winW = event.size.width; winH = event.size.height;
            }
        }
        ...
        if (!paused) {
            ...
            while (curFrame < frames.size()-1 && frames[curFrame+1].time <= simTime)
                curFrame++;
            if (simTime > frames.back().time) curFrame = frames.size()-1;
        }
        if (curFrame >= frames.size()) curFrame = frames.size()-1;
        ...
        if (tEnd - trailStart >= 2) {
            ...
            for (size_t i = colorStart; i < tEnd; i++) {
                ...
                dot.setFillColor(sf::Color(c.r, c.g, c.b,
                    static_cast<sf::Uint8>(alpha * 220)));
        ...
        // Lateral error line
        {
            std::vector<sf::Vector2f> seg = {
                toDisp(f.x_track, f.y_track), toDisp(f.xveh, f.yveh)
            };
        ...
        // Scale bars
        {
            ...
            // Vertical: Y_EXAG display-meters = 1 real meter
            float vBarLen = Y_EXAG;
            sf::RectangleShape vBar(sf::Vector2f(0.4f, vBarLen));
            vBar.setPosition(sbX, sbY);
            vBar.setFillColor(sf::Color(255, 170, 70));
            window.draw(vBar);
            sf::RectangleShape vCap1(sf::Vector2f(1.5f, 0.4f));
            vCap1.setPosition(sbX - 0.55f, sbY - 0.2f);
            vCap1.setFillColor(sf::Color(255, 170, 70));
            window.draw(vCap1);
            sf::RectangleShape vCap2(sf::Vector2f(1.5f, 0.4f));
            vCap2.setPosition(sbX - 0.55f, sbY + vBarLen - 0.2f);
            vCap2.setFillColor(sf::Color(255, 170, 70));
            window.draw(vCap2);
        }
        ...
        {
            sf::Vector2f vc1 = mapToMM(sf::Vector2f(
                camCenter.x + panOff.x - viewW/2.f,
                camCenter.y + panOff.y - viewH/2.f));
            sf::Vector2f vc2 = mapToMM(sf::Vector2f(
                camCenter.x + panOff.x + viewW/2.f,
                camCenter.y + panOff.y + viewH/2.f));
            sf::RectangleShape vRect(sf::Vector2f(
                std::abs(vc2.x - vc1.x), std::abs(vc2.y - vc1.y)));
        ...
        // Y-exag label on minimap
        if (hasFont) {
            sf::Text mmLabel;
            mmLabel.setFont(font);
            mmLabel.setString("Y exag: " + std::to_string(static_cast<int>(Y_EXAG)) + "x");
            mmLabel.setCharacterSize(11);
        ...
        auto drawTaskBox = [&](const char* name, bool active, float periodMs,
                               sf::Color activeColor, float bx, float by,
                               float bw, float bh) {
            sf::Color bgCol = active
                ? sf::Color(activeColor.r/2, activeColor.g/2, activeColor.b/2, 220)
        ...
        auto drawHArrow = [&](float x1, float x2, float y, sf::Color col = sf::Color(120, 120, 150)) {
            sf::Vertex line[] = {
            ...
            window.draw(head, 3, sf::Lines);
        };

        // Lambda to draw a vertical arrow
        auto drawVArrow = [&](float x, float y1, float y2, sf::Color col = sf::Color(120, 120, 150)) {
            sf::Vertex line[] = {
        ...
        drawText("Track View  [" + speedLabel + "]", tx, ty, sf::Color(180, 180, 255), 17);
        ty += lh + 4;

        drawText(fmt(f.time, 3) + " s", tx, ty, sf::Color(200, 200, 200), 22);
        ty += lh + 2;

        drawText("Velocity:  " + fmt(f.velocity, 2) + " m/s", tx, ty);               ty += lh;
        drawText("e_y:       " + fmt(f.e_y, 5) + " m", tx, ty, e_yToColor(f.e_y));    ty += lh;
        drawText("e_y_dot:   " + fmt(f.e_y_dot, 5) + " m/s", tx, ty);                ty += lh;
        drawText("Steering:  " + fmt(f.act_out, 6) + " rad", tx, ty);                 ty += lh;
        drawText("Roll perf: " + fmt(f.rolling_perf, 4), tx, ty);                     ty += lh;
        drawText("Avg perf:  " + fmt(f.avg_perf, 6), tx, ty);                         ty += lh;
        drawText("Violations: " + std::to_string(f.thresh_errors), tx, ty,
                 f.violated ? sf::Color(255, 80, 80) : sf::Color::White);              ty += lh;

        sf::Color critC = f.critical ? sf::Color(255, 200, 50) : sf::Color(80, 200, 80);
        drawText("Section:   " + std::string(f.critical ? "CURVE" : "STRAIGHT"), tx, ty, critC);
        ty += lh;
        bool ffActive = (std::fabs(f.ff_ref_0) > 1e-6 || std::fabs(f.ff_ref_1) > 1e-6);
        drawText("ff_ref:    [" + fmt(f.ff_ref_0, 5) + ", " + fmt(f.ff_ref_1, 5) + "]",
        ...
        // ── Help bar ──
        drawText("Space=Pause  R=Restart  F=Follow  Up/Down=Speed  Scroll=Zoom  RightDrag=Pan  Esc=Quit",
        ...
        // ── Scale labels (screen space) ──
        {
            ...
            drawText("50 m (horizontal)", scaleX, scaleY - 16.f, sf::Color(200, 200, 200), 12);
            drawText("1 m (vertical, true scale)",
                     scaleX + 14.f, scaleY - static_cast<float>(Y_EXAG) * 0.8f - 4.f,
                     sf::Color(255, 170, 70), 11);
        }
